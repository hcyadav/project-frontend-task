...
// Dummy data from employee-dashboard.md
...
export const getEmployeeById = async (id: number): Promise<Employee | undefined> => {
  return new Promise((resolve) => {
    setTimeout(() => resolve(employees.find(e => e.id === id)), 300);
  });
};

// Track the last assigned ID to ensure strict sequential generation
let lastId = Math.max(...employees.map(e => e.id), 0);
...
export const updateEmployee = async (id: number, data: UpdateEmployeeInput): Promise<Employee> => {
